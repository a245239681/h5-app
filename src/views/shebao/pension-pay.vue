<template>
  <div class="prove-list payment-details">
    <van-skeleton :row="6" :loading="loading">
      ...
      <div
        v-show="showTips === false"
        v-for="(item, inx) in list"
        :key="inx"
        class="cognizance"
      >
        <div class="list-date fw">{{ item.date_time }}</div>
        <van-cell-group v-for="(items, i) in item.item" :key="i">
          <van-cell title="险种" :value="items.xz" />
          <van-cell title="缴费基数" :value="items.jfjs" />
          <van-cell title="单位实缴本金" :value="items.a12a13" />
          <van-cell title="个人实缴本金" :value="items.a6a8" />
          <van-cell title="到账状态" :value="items.aae078" />
          <van-cell title="经办机构" :value="items.orgid" />
        </van-cell-group>
      </div>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      that.$http.postJson('/api/appweb/Personal/jgAnnuityBill', data, res => {
        if (res.data.code === 0) {
